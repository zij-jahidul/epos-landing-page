...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $subscriptions = Subscription::where('created_at', '>=', Carbon::now()->subDays(30))
            ...
            ->whereMonth('created_at', date('m'))
            ->groupByRaw("DAY(created_at)")
            ->orderByRaw("DAY(created_at)") // Use DAY(created_at) in the ORDER BY
            ->pluck('total_subscriptions', 'per_day');
        ...
        $userLoginsReport = UserLogin::selectRaw("COUNT(*) as created_at_count, DATE(created_at) as date_name")
            ->groupByRaw("DATE(created_at)")
            ->orderByRaw("DATE(created_at) DESC") // Use DATE(created_at) in the ORDER BY
            ->limit(10)
            ->pluck('created_at_count', 'date_name');
